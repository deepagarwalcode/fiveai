...
const LogoParticles = () => {
  ...
  for (let i = 0; i < particles.maxCount; i++) {
    ...
    if (i3*factor < originalArray.length) {
      newArray[i3 + 0] = originalArray[i3*factor + 0] / 10 + sizesArray[i] * randomnessOffset;
      newArray[i3 + 1] = originalArray[i3*factor + 1] / 10 + sizesArray[i] * randomnessOffset;
      newArray[i3 + 2] = originalArray[i3*factor + 2] / 10 + sizesArray[i] * randomnessOffset;
    } else {
      const randomIndex = Math.floor(position.count * Math.random()) * 3;
      newArray[i3 + 0] = originalArray[randomIndex + 0] / 10 + sizesArray[randomIndex] * randomnessOffset;
      newArray[i3 + 1] = originalArray[randomIndex + 1] / 10 + sizesArray[randomIndex] * randomnessOffset;
      newArray[i3 + 2] = originalArray[randomIndex + 2] / 10 + sizesArray[randomIndex] * randomnessOffset;
    }
  }
